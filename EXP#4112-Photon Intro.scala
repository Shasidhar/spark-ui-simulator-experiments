...
sc.setJobDescription("Step C: UDFs. Photon doesn't support UDFs yet.")
...
val sourceFile = "wasbs://spark-ui-simulator@dbacademy.blob.core.windows.net/global-sales/transactions/2011-to-2018-100gb.csv"

sc.setJobDescription("Step D: CSV Project and Drop a column. Doesn't use photon.")

spark
  .read.schema(fullSchema)                                     // Specifying all columns
  .csv(sourceFile)                                             // Load the transactions table
  .select("trx_id", "retailer_id", "city_id", "transacted_at") // Select 4 columns
  .drop("transacted_at")                                       // Drop the 4th column
  .write.format("noop").mode("overwrite").save()               // Test with a noop write
